...
def genera_filtro(num_particulas, balizas, real, centro=[2,2], radio=3):
  ...
  for i in range(num_particulas):
    particula = robot()
    x = (centro[0] - radio) + (random.random() * radio * 2)
...
def dispersion(filtro):
  ...
  for particula in filtro:
    distancias.append(sqrt(pow(particula.x, 2) + pow(particula.y, 2)))
  media = sum(distancias) / len(filtro)
  
  desviaciones = []
  for dist in distancias:
    desviaciones.append(pow(media - dist, 2))
  varianza = sum(desviaciones) / len(filtro)
...
def peso_medio(filtro):
  ...
  for particula in filtro:
    pesos.append(particula.measurement_prob(medidas, objetivos))
  
  media = sum(pesos) / len(filtro)
  return media 
...
filtro = genera_filtro(N_INICIAL, objetivos, real, [real.x, real.y], 1)

trayectoria = [real.pose()]
...
for punto in objetivos:
  while distancia(trayectoria[-1],punto) > EPSILON and len(trayectoria) <= 1000:
    ...
    # Muevo todas las particulas como si fueran el robot.
    for particula in filtro:
      if HOLONOMICO:
        particula.move(w,v)
      else:
